...
class InterbotixArmXSInterface:
    ...
    def publish_positions(self, positions, moving_time=None, accel_time=None, blocking=True):
        ...
        joint_commands = interbotix_msgs.JointGroupCommand(self.group_name, self.joint_commands)
    ...
    def check_joint_limits(self, positions):
        ...
        speed_list = [
            abs(goal - current) / float(self.moving_time) for goal, current in zip(theta_list, self.joint_commands)
        ]
        ...
        for x in range(self.group_info.num_joints):
            ...
            if speed_list[x] > self.group_info.joint_velocity_limits[x]:
                ...
                rospy.logwarn("Limit is %f, value was %f." % (self.group_info.joint_velocity_limits[x], theta_list[x]))
                return False
        return True
    ...
    def set_ee_cartesian_trajectory(
        ...
        for i in range(N + 1):
            ...
            if success:
                ...
                rospy.loginfo(
                    "%.1f%% of trajectory successfully planned. Trajectory will not be executed." % (i / float(N) * 100)
                )
                break
        ...
        if success:
            ...
            self.core.pub_traj.publish(interbotix_msgs.JointTrajectoryCommand("group", self.group_name, joint_traj))
    ...
    def get_ee_pose(self):
        joint_states = [
            self.core.joint_states.position[self.core.js_index_map[name]] for name in self.group_info.joint_names
        ]
