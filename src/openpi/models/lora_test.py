...
import openpi.models.transformer as transformer
...
def eqn_to_shape(eqn: str, annotation_to_dim_size: dict[str, int]) -> tuple[tuple[int, ...], ...]:
    (lhs_part_0, lhs_part_1), _ = transformer._parse_einops_eqn(eqn)
...
def test_lora_einsum_equivalent_to_original(eqn: str, lora_annotation: str):
    ...
    einsum = transformer.Einsum(shape=w_shape, name="einsum", init_fn=nn.initializers.lecun_normal())
    lora_einsum = transformer.LoRAEinsum(
        einsum,
        transformer.LoRAConfig(rank=4, alpha=4.0),
        lora_annotation,
...
def test_lora_einsum_param_merge_works(eqn: str, lora_annotation: str):
    ...
    lora_einsum_variables = lora_einsum.init(jax.random.PRNGKey(0), x, method=module_call)
    einsum_variables = transformer.merge_lora_params(lora_einsum_variables, lambda x: lora_annotation)
